...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        #TODO ADD config
        if hasattr(config, "use_delta") and config.delta_type == "adapter":
            self.attn_adapter = AdapterLayer(hidden_size=self.embed_dim, bottleneck_dim=24, non_linearity='gelu_new')
            self.fc2_adapter = AdapterLayer(hidden_size=self.embed_dim, bottleneck_dim=24, non_linearity='gelu_new')
...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and logits.shape[1] != labels.shape[1]:
            prompt_len = logits.shape[1] - labels.shape[1]
