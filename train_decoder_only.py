...
from examples_seq2seq.gpt_seq2seq_trainer import GPTSeq2SeqTrainer
...
class MyTrainingArgument:
    ...
    metaicl_checkpoint: str = field(
        default=None, metadata={"help": "Used in metaicl setting"}
    )
    ...
    # unfrozen_modules :str = field(
    #     default_factory=lambda:["deltas"], metadata={"help":"default opendelta config"},
    # )
...
def main():
    ...
    parser.add_argument("--do_icl", default=False, action="store_true")
    parser.add_argument("--icl_dataset_seed", type=int, default=None)
    parser.add_argument("--icl_k_examples", type=int, default=None)

    parser.add_argument("--extract_neuron", default=False, action="store_true", help="shape as [layer_num, 4096]")
    parser.add_argument("--extract_last_hidden", default=False, action="store_true", help="the first ans token shape as [task_len, 768]")
    parser.add_argument("--label_mapping_type", type=str, default=None, help=" select in number or original ")

    parser.add_argument("--reset_pos_id_for_instance", default=False, action="store_true", help="set pos id for instance from 0")
    parser.add_argument("--used_task", type=str, default=None, help="only used in icl no_text setting")
    ...
    # sentinal check
    assert ((delta_args.do_icl and delta_args.icl_dataset_seed and delta_args.icl_k_examples and not training_args.do_train) or
    (not delta_args.do_icl and not delta_args.icl_dataset_seed and not delta_args.icl_k_examples))

    assert (delta_args.extract_neuron and not training_args.do_train) or not delta_args.extract_neuron
    assert (delta_args.extract_last_hidden and not training_args.do_train) or not delta_args.extract_last_hidden
    # assert (delta_args.extract_neuron ^ delta_args.extract_last_hidden) or (delta_args.extract_neuron == delta_args.extract_last_hidden == False), "specify only one argument please"

    if delta_args.label_mapping_type is not None:
        assert delta_args.label_mapping_type in ["number", "original"]

    if delta_args.metaicl_checkpoint is None and "gpt2" in model_args.model_name_or_path:
        three_newlines = False
    else:
        three_newlines = True

    logger.warning(f"three_newlines is {three_newlines}, check if it is expected    !!!!!!!!!!!!!!!!")
    ...
    try:
        ...
        #now bloom
        generation_config = GenerationConfig.from_pretrained("facebook/opt-125m")
    ...
    if delta_args.dropout_rate is not None:
        ...
        config.attn_pdrop = delta_args.dropout_rate
        config.embd_pdrop = delta_args.dropout_rate
        config.resid_pdrop = delta_args.dropout_rate
        logger.info("we set all the possible dropout_rate in config to {}".format(config.dropout_rate))
    ...
    if delta_args.metaicl_checkpoint is not None:
        logger.warning(f"load meta icl checkpoint at path : {delta_args.metaicl_checkpoint} !!!!!!!!!!!!!!!")
        model.load_state_dict(torch.load(delta_args.metaicl_checkpoint))
    # only used in gpt2
    # tokenizer.add_special_tokens({"pad_token": "[PAD]"})
    tokenizer.padding_side = "left"
    tokenizer.truncation_side = "left"
    logger.warning(f"we set padding_side and truncation_side to {tokenizer.padding_side} and {tokenizer.truncation_side} respectively !!!!!!")
        ...
        if delta_args.delta_type == "apet":
            ...
            while index < len(delta_param):
                if "lora" in delta_param[index][0]:
                    ...
                    index += 4
                elif "soft_prompt" in delta_param[index][0]:
                    all_param.append(delta_param[index: index + 1])
                    index += 1
                else:
                    raise ValueError("find no lora, adapter, prompt")
    ...
    def preprocess_function(examples, max_target_length):
        ...
        # labels are shifted in model.forward
        # should set labels = input_ids
        model_inputs["labels"] = copy.deepcopy(model_inputs["input_ids"])


        # prompt_setting using -1 as placeholder
        if hasattr(config, "use_delta"):
            if config.delta_type == "soft_prompt":

                for idx in range(len(model_inputs["input_ids"])):
                    model_inputs["input_ids"][idx] = [-1] * config.soft_token_num + model_inputs["input_ids"][idx]
                    model_inputs["attention_mask"][idx] = [1] * config.soft_token_num + model_inputs["attention_mask"][idx]


        for idx in range(len(model_inputs["labels"])):
            model_inputs["labels"][idx] = list(map(lambda x: -100 if x == tokenizer.pad_token_id else x, model_inputs["labels"][idx]))


        if delta_args.reset_pos_id_for_instance and delta_args.delta_type is None:
            # reset pos id for test instance id : [icl 0,1,2,3,...., test 0,1,2,...]
            # we do not want the pos id calc in model, so we past in advance
            assert examples["icl_test_source"][0] is not None

            icl_test_source_inputs = tokenizer([s for s in examples['icl_test_source']], max_length=data_args.max_source_length,
                                 padding=False, truncation=True)

            pos_ids = torch.cumsum(torch.tensor(model_inputs["attention_mask"]), dim=-1) - 1
            pos_ids = pos_ids.masked_fill_(torch.tensor(model_inputs["attention_mask"]) == 0, 1)

            for index, cur_icl_test_source_inputs in enumerate(icl_test_source_inputs["attention_mask"]):
                cur_icl_test_source_pos_ids = torch.cumsum(torch.tensor(cur_icl_test_source_inputs), dim=-1) - 1
                pos_ids[index][-cur_icl_test_source_pos_ids.shape[-1]:] = cur_icl_test_source_pos_ids

            pos_ids = np.array(pos_ids).tolist()


            model_inputs["position_ids"] = pos_ids

        elif delta_args.reset_pos_id_for_instance and delta_args.delta_type == "soft_prompt":
            pos_ids = torch.cumsum(torch.tensor(model_inputs["attention_mask"]), dim=-1) - 1
            pos_ids = pos_ids.masked_fill_(torch.tensor(model_inputs["attention_mask"]) == 0, 1)
    ...
    if training_args.do_eval:
        eval_datasets = {eval_dataset: AutoTask.get(eval_dataset,
                                                    ...
                                                    icl_dataset_seed=delta_args.icl_dataset_seed,
                                                    icl_k_examples=delta_args.icl_k_examples,
                                                    three_newlines=three_newlines,
                                                    reset_pos_id_for_instance=delta_args.reset_pos_id_for_instance,
                                                    used_task=delta_args.used_task).get(
            split="validation",
            split_validation_test=training_args.split_validation_test,
            add_prefix=False,
    ...
    assert training_args.generation_max_length >= 128 and training_args.generation_max_length > data_args.max_source_length, "decoder_only scripts need a longer target length, remember remove .label_list in tasks.py"
        ...
        if 'LOCAL_RANK' in os.environ and os.environ['LOCAL_RANK'] == '0':
            ...
            with open(os.path.join(training_args.output_dir, 'wrong_text.txt'), 'w', encoding='utf-8') as f:
                for pre, grn in zip(preds, data_info):
                    if pre not in grn or (type(pre) == type(grn) and pre != grn):
    ...
    def post_processing_function_classification(
        ...
        used_labels_ids = [tokenizer.encode(option)[0] for option in options]

        logits = torch.argmax(torch.tensor(preds[:, used_labels_ids]), axis=-1)

        # important!  label_ids = input_ids due to gpt nature, so do not use label_ids to calculate accuracy
        # lazy hack
        if "ed2lm" in data_args.eval_dataset_name[0].lower():
            ...
            formatted_predictions = []
            for ans in logits:
                formatted_predictions.append(options[ans])

        # we do not need label_ids
        # EvalPrediction(predictions=formatted_predictions, label_ids=outputs.label_ids,
        #                data_info=features["extra_fields"])
        return EvalPrediction(predictions=formatted_predictions, label_ids=None, data_info=features["extra_fields"])


    def post_processing_function_decoder_only(
            examples, features, outputs, stage="eval"
    ):
        # Decode the predicted tokens.
        preds = outputs.predictions
        ...
        new_token_index = len(features["input_ids"][0])
        decoded_preds = tokenizer.batch_decode(preds[:,new_token_index:], skip_special_tokens=True)
        ...
        if "ed2lm" in data_args.eval_dataset_name[0].lower():
            ...
            for idx, _ in enumerate(decoded_preds):
                if isinstance(decoded_preds[idx], str):
                    ...
                    decoded_preds[idx] = decoded_preds[idx].split("Output: ")[-1].strip()
                    # decoded_preds[idx] = decoded_preds[idx].split(" ")[0].strip()
    ...
    def post_processing_function_decoder_only_for_first_token(
            examples, features, outputs, stage="eval"
    ):
        # a less rigorous evaluation method that we only compare the first generated token
        # cause sometime icl can not generate the whole vocab
        # Decode the predicted tokens.
        preds = outputs.predictions
        if isinstance(preds, tuple):
            preds = preds[0]
        ...
        new_token_index = len(features["input_ids"][0])
        decoded_preds = tokenizer.batch_decode(preds[:,new_token_index:new_token_index + 1], skip_special_tokens=True)
        ...
        if "ed2lm" in data_args.eval_dataset_name[0].lower():
            ...
            extra_fields = copy.deepcopy(features["extra_fields"])
            for idx, _ in enumerate(decoded_preds):
                if isinstance(decoded_preds[idx], str):
                    decoded_preds[idx] = decoded_preds[idx].strip()
                ...
                extra_fields[idx] = tokenizer.decode(tokenizer.encode(extra_fields[idx])[0])


            formatted_predictions = decoded_preds

        return EvalPrediction(predictions=formatted_predictions, label_ids=outputs.label_ids, data_info=extra_fields)

    if delta_args.label_mapping_type == "number":

        post_process_function = post_processing_function_decoder_only
        logger.warning("we use default decoder_only metrics for evaluation!!!!!!!!")

    elif delta_args.label_mapping_type == "original":

        # post_process_function = post_processing_function_decoder_only_for_first_token
        # logger.warning("we use a smooth one to compute decoder_only metrics for evaluation "
        #                "that we only compute the first token")
    ...
    if training_args.predict_with_generate == False:
        ...
        post_process_function = post_processing_function_classification

        logger.warning("overwrite post_process_function to post_processing_function_classification")

    # Initialize our Trainer
    trainer = GPTSeq2SeqTrainer(
        model=model,
        args=training_args,
        ...
        extract_neuron_or_hidden=delta_args.extract_neuron or delta_args.extract_last_hidden,
    ...
    if training_args.do_eval:
        ...
        if delta_args.extract_neuron:
            logger.warning("extract_neuron, setting register_forward_hook")
            ...
            neuron_outputs = [[] for _ in range(layer_num)]

            def save_forward_hook(n):
                def fn(_, __, output):
                    # print("=====")
                    # print(output)
                    # print("----")
                    ...
                    # print("=====")
                    # exit()
                    # we only extract the last input token's neuron
                    # for prompt output.shape[1] == data_args.max_source_length do not work well
                    if output.shape[1] != 1:

                        # if len(neuron_outputs[n]) == 0:
                        #     # pool batch dim
                        #     neuron_outputs[n].append(torch.sum(output.detach().to("cpu"), dim=0).contiguous())
                        #     neuron_outputs[n].append(0)
                        # else:
                        #     neuron_outputs[n][0] += torch.sum(output.detach().to("cpu"), dim=0).contiguous()
                        #     neuron_outputs[n][1] += 1

                        # do not pool batch dim
                        neuron_outputs[n].append(output.detach().to("cpu"))

                    else:
                        # TODO check if we choose the correct token here
                        pass
            ...
            for n in range(layer_num):
                try:
                    # gpt-neo
                    model.transformer.h[n].mlp.c_fc.register_forward_hook(save_forward_hook(n))
                except:
                    # gpt-j
                    model.transformer.h[n].mlp.fc_in.register_forward_hook(save_forward_hook(n))


        if delta_args.extract_last_hidden:
            logger.warning("extract_last_hidden, setting register_forward_hook")
            ...
            def save_forward_hook():
                def fn(_, __, output):
                    # we only extract the last input token's neuron
                    # for prompt, can not write output.shape[1] == data_args.max_source_length, coz it does not work well
                    if output.shape[1] != 1:
                        last_hidden_outputs.append(output[:, -1, :].detach().to("cpu").contiguous())
            ...
            model.lm_head.register_forward_hook(save_forward_hook())
        ...
        if delta_args.extract_neuron:
            # merge
            final_neuron_outputs = []
            for k in range(layer_num):
                # outputs[k] = relu(np.concatenate(outputs[k]))
                # outputs[k] = torch.relu(torch.cat(outputs[k]))

                ### previous pool solution
                # since we have already plus them together
                # neuron_outputs[k][0] /= neuron_outputs[k][1]
                # final_neuron_outputs.append(neuron_outputs[k][0])
                neuron_outputs[k] = torch.cat(neuron_outputs[k])

            # [layer_num, dev_set, input_length, hidden], [24, 100, 1014, 3072]
            final_neuron_outputs = torch.stack(neuron_outputs)
            # [24, 100, 3072]
            final_neuron_outputs = final_neuron_outputs[:, :, -1, :].contiguous()
            # print(neuron_outputs)
            # [size [512, 768], dev_set_size ]
            # print(final_neuron_outputs)
            # [size [512, 768] * 12]
            # final_neuron_outputs = torch.stack(final_neuron_outputs)
            # size [12,512,768]
            ...
            # save
            saved_neuron_path = os.path.join(training_args.output_dir, "saved_neuron.bin")
            torch.save(final_neuron_outputs, saved_neuron_path)
            print(final_neuron_outputs)
            logger.warning(f"save final neuron outputs with shape : {final_neuron_outputs.shape} to {saved_neuron_path} successfully !")

        if delta_args.extract_last_hidden:

            final_last_hidden_outputs = torch.cat(last_hidden_outputs,dim=0)
            saved_last_hidden_path = os.path.join(training_args.output_dir, "saved_last_hidden.bin")
            torch.save(final_last_hidden_outputs, saved_last_hidden_path)

            print(final_last_hidden_outputs)
            logger.warning(
                f"save final neuron outputs with shape : {final_last_hidden_outputs.shape} to {saved_last_hidden_path} successfully !")
